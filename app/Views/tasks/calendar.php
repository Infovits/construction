...
<?= $this->section('title') ?>Task Calendar<?= $this->endSection() ?>
...
<style>
    ...
    .task-count-badge.urgent  { background: #c026d3; }
    .task-count-badge.high    { background: #ef4444; }
    .task-count-badge.medium  { background: #f59e0b; }
    .task-count-badge.low     { background: #10b981; }
    ...
    @media (max-width: 768px) {
        ...
        .task-count-badge { width: 22px; height: 22px; font-size: 0.75rem; }
    }
</style>
            ...
            <div class="lg:col-span-2 bg-white rounded-xl shadow border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div class="bg-blue-50 p-4 rounded-lg text-center">
                        ...
                        <p class="text-2xl font-bold text-blue-900" id="totalTasks">0</p>
                    </div>
                    <div class="bg-green-50 p-4 rounded-lg text-center">
                        <p class="text-sm text-green-700">Completed</p>
                        <p class="text-2xl font-bold text-green-900" id="completedTasks">0</p>
                    </div>
                    <div class="bg-amber-50 p-4 rounded-lg text-center">
                        <p class="text-sm text-amber-700">Active</p>
                        <p class="text-2xl font-bold text-amber-900" id="activeTasks">0</p>
                    </div>
                    <div class="bg-red-50 p-4 rounded-lg text-center">
                        <p class="text-sm text-red-700">Overdue</p>
                        <p class="text-2xl font-bold text-red-900" id="overdueTasks">0</p>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            <div class="px-6 py-5 sm:px-8 sm:py-6 border-b border-gray-200 bg-gray-50">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900" id="calendarTitle">Task Calendar</h1>
                        <p class="mt-1 text-gray-600" id="eventCount">0 tasks loaded</p>
                    </div>
                </div>
...
<!-- Day Tasks Modal -->
<div id="dayTasksModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-xl shadow-2xl max-w-lg w-full mx-4 max-h-[85vh] overflow-y-auto">
        <div class="p-6">
            <div class="flex justify-between items-center mb-5">
                <h3 class="text-xl font-bold text-gray-900" id="modalDateTitle">Tasks on …</h3>
                <button id="closeDayModal" class="text-2xl text-gray-500 hover:text-gray-700">×</button>
            </div>
            <div id="dayTasksList" class="space-y-3"></div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: function(fetchInfo, successCallback) {
            fetch(`<?= base_url('admin/tasks/api/calendar-events') ?>?start=${fetchInfo.startStr}&end=${fetchInfo.endStr}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('eventCount').textContent = `${allTasks.length} tasks`;

                    const events = allTasks.map(t => ({
                        id: t.id,
                        title: t.title,
                        start: t.planned_start_date,
                        end: t.planned_end_date || t.planned_start_date,
                        allDay: true,
                        classNames: [getStatusClass(t.status)],
                        extendedProps: { priority: t.priority || 'low' }
                    }));
        ...
        dateClick: function(info) {
            ...
            const dayTasks = allTasks.filter(t => t.planned_start_date?.startsWith(dateStr));

            if (dayTasks.length === 0) return;

            document.getElementById('modalDateTitle').textContent = `Tasks on ${dateStr}`;
            ...
            dayTasks.forEach(task => {
                ...
                const projectName   = task.project_name   || task.project_title   || task.project   || '—';
                const assignedTo    = task.assigned_name || task.assigned_to_name || task.assignee_name || task.assigned_to || task.user_name || 'Unassigned';
                ...
                div.innerHTML = `
                    ...
                    <div class="flex-1 min-w-0">
                        <a href="<?= base_url('admin/tasks/') ?>${task.id}" class="font-semibold text-gray-900 hover:text-indigo-600 truncate block">${task.title}</a>
                        ...
                        <div class="text-sm mt-2 text-gray-700 space-y-1">
                            ...
                            <div><span class="font-medium text-gray-600">Priority:</span> ${task.priority || '—'}</div>
                        </div>
                    </div>
                `;
        ...
        datesSet: () => setTimeout(() => addBadges(allTasks), 80)
    });
    ...
    document.getElementById('dayTasksModal').onclick = e => {
        if (e.target === document.getElementById('dayTasksModal')) {
            document.getElementById('dayTasksModal').classList.add('hidden');
        }
    };
    ...
    function updateStats(tasks) {
        ...
        const overdue = tasks.filter(t =>
            t.planned_end_date &&
            new Date(t.planned_end_date) < new Date() &&
            t.status !== 'completed'
        ).length;
    ...
    function addBadges(tasks) {
        document.querySelectorAll('.task-count-badge').forEach(el => el.remove());

        document.querySelectorAll('.fc-daygrid-day').forEach(day => {
            const date = day.dataset.date;
            const dayTasks = tasks.filter(t => t.planned_start_date?.startsWith(date));
            if (dayTasks.length === 0) return;

            let priority = 'low';
            if (dayTasks.some(t => t.priority === 'urgent')) priority = 'urgent';
            else if (dayTasks.some(t => t.priority === 'high')) priority = 'high';
            else if (dayTasks.some(t => t.priority === 'medium')) priority = 'medium';

            const badge = document.createElement('div');
            badge.className = `task-count-badge ${priority}`;
